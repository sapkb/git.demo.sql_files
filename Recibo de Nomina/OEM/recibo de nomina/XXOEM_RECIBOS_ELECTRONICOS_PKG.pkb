...
--                                          P_RECIBO_INI           NUMBER
   ...
   IS
     ...
     */
      ...
      CURSOR c_empleados
      IS  
      select id_nomina,numero_empleado,fecha_pago,fecha_inicial,fecha_final,
         dias_pagados,salario_Base_Cot,sum(SDI) SDI,time_period_id,
         person_id,payroll_action_id,decripcion
  from ( 
  SELECT   distinct(ppe.EMPLOYEE_NUMBER) numero_empleado,
          ...
          nvl( ( select PESTL.element_set_name 
        from
            --pay_payroll_actions ppa
            pay_element_sets PES
            ,pay_element_sets_tl PESTL
        where 1=1
        AND PPA.ELEMENT_SET_ID 
            = PES.ELEMENT_SET_ID 
...
GROUP BY   pay.payroll_id,
           ...
           pca.segment6,
           pca.segment7,
           PAS.SOFT_CODING_KEYFLEX_ID--,piv.INPUT_VALUE_ID,prv.RUN_RESULT_ID
                                     --,prv.RUN_RESULT_ID
           ,
           apps.XXOEM_RECIBOS_ELECTRONICOS_PKG.F_REGRESA_SDI (piv.INPUT_VALUE_ID,
                                                         prv.RUN_RESULT_ID)
...
group by id_nomina,numero_empleado,fecha_pago,fecha_inicial,fecha_final,
         dias_pagados,salario_Base_Cot,time_period_id,
         person_id,payroll_action_id,decripcion
...
--             rpad(c_emp.fecha_final,9,' ')||
--             rpad(c_emp.dias_pagados,3,' ')||
--             rpad(c_emp.salario_Base_Cot,5,' '));
--             
             
             --imp_l(g_xml('SDI',c_emp.SDI));
--             imp_l(rpad(c_emp.decripcion,50,'-'));
        
--        
--         IMP_O('  <EMPLEADOS>');
--             imp_o(g_xml('IDH','H'));
--             imp_o(g_xml('ID_NOMINA',c_emp.payroll_action_id));
--             imp_o(g_xml('NUMERO_EMPLEADO',c_emp.numero_empleado));
--             imp_o(g_xml('FECHA_PAGO',c_emp.fecha_pago));
--             imp_o(g_xml('FECHA_INICIAL',c_emp.fecha_inicial));
--             imp_o(g_xml('FECHA_FINAL',c_emp.fecha_final));
...
--           
         ...
         INSERT INTO XXOEM_PAY_MAESTRO_RECIBOS2 (recibo_id,   ---enlasa tablas
                                                 ID_NOMINA,
                                                 numero_empleado, ----requerido
                                                 fecha_pago,
                                                 fecha_inicial,
                                                 fecha_final,
                                                 dias_pagados,
...
--             imp_o(g_xml('DESCRIPCION',c_emp.decripcion));
            ...
            --IMP_O('  </PERCEPCIONES>');
               INSERT INTO xxoem.XXOEM_PAY_RECIBO_PERCEPCIONES2 (
                                                                    RECIBO_ID,
                                                                    NUM_LINEA,
                                                                    CLAVE,--CONCEPTO,
                                                                    TIPO_PERCEPCION,
...
--           IMP_O('  </DEDUCCIONES>');
               ...
               INSERT INTO xxoem.XXOEM_PAY_RECIBO_DEDUCCIONES2 (
                                                                   RECIBO_ID,
                                                                   NUM_LINEA,
                                                                   CONCEPTO,
                            --                                       IMPORTE,
...
--            IMP_O('  </INCAPACIDADES>');
               ...
               INSERT INTO XXOEM.XXOEM_PAY_RECIBO_indem2 (   RECIBO_ID,
                                                                   NUM_LINEA,
                                                                   CONCEPTO,
                                                                   IMPORTE,
                             --                                      IMPORTE_DEDUCCIONES,
       ...
       -- Genera los registros para las incapacidades
         ...
         LOOP
            ...
            -------------SACA TIPO DE HORAS-----
            IF (C_EXT.CONCEPTO LIKE '%DOBLES') THEN
            TIPO_HORAS:='DOBLES';
            
            ELSIF (C_EXT.CONCEPTO LIKE '%TRIPLES') THEN
            TIPO_HORAS:='TRIPLES';
            END IF;
...
--             imp_o(g_xml('DESCRIPCION',c_emp.decripcion));
--            IMP_O('  </HORAS_EXTRA>');
               INSERT INTO XXOEM.XXOEM_PAY_RECIBO_EXTRAS2 (   RECIBO_ID,
                                                                   NUM_LINEA,
                                                                   CONCEPTO,
                                                                   IMPORTE_PAGADO,
                             --                                      IMPORTE_DEDUCCIONES,
...
--         INSERT INTO XXOEM_TEMPORAL_JM2
--           VALUES   (
--                           l_recibo_id1
--                        || '-B-'
--                        || l_recibo_id2
   ...
   BEGIN
      SELECT   DEFINED_BALANCE_ID
       ...
       WHERE   (   (business_group_id = 81)
               ...
               AND (BALANCE_NAME_AND_SUFFIX LIKE
                       'Dias Cuota Seguridad Social 1_ASG_GRE_RUN');
               ...
               into P_SDI
                from  
                 ...
                 where 1=1
                   ...
                   AND ETYTL.LANGUAGE = USERENV ('LANG')
                   AND (ETYTL.ELEMENT_NAME = 'I7001 SDI')
